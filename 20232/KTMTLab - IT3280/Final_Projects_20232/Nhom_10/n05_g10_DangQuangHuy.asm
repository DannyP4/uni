.data
	...
	errorMsg: .asciiz "Input sai cu phap"
	startMsg: .asciiz "Hay nhap chuoi infix\nNote: chi chua + - * / % ()\ncac so tu 00-99"
...
# khoi tao cac trang thai
	li $s7,0					# bien trang thai $s7  luu cac trang thai dau vao
...
# kiem tra dau vao
	...
	beq $t9, 0, digit1				# Neu trang thai la 0 => check xem co phai nhan chu so hay k
	beq $t9, 1, digit2				# Neu trang thai la 1 => co 1 chu so check xem co nhan tiep hay k
	beq $t9, 2, digit3				# neu trang thai la 2 => co 2 chu so check xem co nhan tiep hay k
	continueScan:
	...
	beq $t4, '(', openBracket
	beq $t4, ')', closeBracket
wrongInput:							# xu li khi dau vao loi
	li $v0, 55
 	la $a0, errorMsg
 	li $a1, 2
 	syscall
...
printPostfix:
	...
	bgt $t6,$t5,finishPrint			# in ra postfix xong roi tinh ket qua
...
CalculatorPost:	
	...
	bgt $t7,99,calculate			# neu gia tri postfix hien tai > 99 --> toan tu --> lay ra 2 toan hang va tinh toan
...
PopOperatorToPostfix:			# lay toan tu o dinh va luu vao postfix
	...
	addi $t7,$t7,100			# ma hoa toan tu + 100 de tranh trung voi cac so
