...
Check_Label:
	...
	addi	$t1, $t1, 1
Loop_Label:
	add $a2, $s2, $t1  		# Lấy địa chỉ của ký tự tiếp theo từ hàng đợi
	lb $t2, 0($a2)    		# Lấy ký tự tiếp theo và lưu vào $t2
	beq $t2, $zero, True_Label     	# Nếu gặp ký tự NULL, nhảy đến True_Label
	li $t5, 10                     	# t5 = 'New line'
	beq $t2, $t5, True_Label	# Nếu gặp ký tự newline, nhảy đến True_Label
	
	slti $t4, $t2, 48
	bne $t4, $zero, False_code       # Nếu ký tự nhỏ hơn '0' (mã ASCII 48), nhảy đến False_code
	li $t5, 58
	beq $t2, $t5, False_code         # Nếu ký tự là ':', nhảy đến False_code
	li $t5, 59
	beq $t2, $t5, False_code         # Nếu ký tự là ';', nhảy đến False_code
	li $t5, 60
	beq $t2, $t5, False_code         # Nếu ký tự là '<', nhảy đến False_code
	...
	beq $t2, $t5, False_code         # Nếu ký tự là '`', nhảy đến False_code
	slti $t4, $t2, 123
	beq $t4, $zero, False_code       # Nếu ký tự lớn hơn 'z' (mã ASCII 122), nhảy đến False_code
